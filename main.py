...
import numpy as np
from utils.metric import get_ner_fmeasure, get_metrics
...
def evaluate(data, model, name, nbest=None):
    ...
    save_words, save_labels, save_preds = [], [], []
    ...
    for batch_id in range(total_batch):
        ...
        batch_text = [sent[0] for sent in instance_text]
        save_words += batch_text
        if not instance:
            continue
        batch_word, batch_features, batch_wordlen, batch_wordrecover, batch_char, batch_charlen, batch_charrecover, batch_label, mask, gazs, t_graph, c_graph, l_graph   = batchify_with_label(instance, data.HP_gpu, False, data.sentence_classification)
        if nbest and not data.sentence_classification:
            scores, nbest_tag_seq = model.decode_nbest(batch_word,batch_features, batch_wordlen, batch_char, batch_charlen, batch_charrecover, mask, nbest, gazs, t_graph, c_graph, l_graph)
            ...
            tag_seq = model(batch_word, batch_features, batch_wordlen, batch_char, batch_charlen, batch_charrecover, mask, gazs, t_graph, c_graph, l_graph)
        # print("tag:",tag_seq)
        ...
        #gmb 
        #allowed_label_list = ['O', 'B-geo', 'B-gpe', 'B-per', 'I-geo', 'B-org', 'I-org', 'B-tim', 'B-art', 'I-art', 'I-per', 'I-gpe', 'I-tim', 'B-nat', 'B-eve', 'I-eve', 'I-nat']
        #re3d
        #allowed_label_list = ['O', 'B-Organisation', 'I-Organisation', 'B-Temporal', 'I-Temporal', 'B-Nationality', 'B-Location', 'I-Location', 'B-Person', 'I-Person', 'B-Weapon', 'I-Weapon', 'B-DocumentReference', 'I-DocumentReference', 'B-Money', 'I-Money', 'B-Quantity', 'B-MilitaryPlatform', 'I-MilitaryPlatform', 'I-Quantity', 'I-Nationality']
        #movie
        #allowed_label_list = ['O', 'B-ACTOR', 'I-ACTOR', 'B-YEAR', 'B-TITLE', 'B-GENRE', 'I-GENRE', 'B-DIRECTOR', 'I-DIRECTOR', 'B-SONG', 'I-SONG', 'B-PLOT', 'I-PLOT', 'B-REVIEW', 'B-CHARACTER', 'I-CHARACTER', 'B-RATING', 'B-RATINGS_AVERAGE', 'I-RATINGS_AVERAGE', 'I-TITLE', 'I-RATING', 'B-TRAILER', 'I-TRAILER', 'I-REVIEW', 'I-YEAR']
        #rest
        
        #allowed_label_list = ['O', 'B-Rating', 'I-Rating', 'B-Location', 'I-Location', 'B-Amenity', 'B-Cuisine', 'B-Hours', 'I-Hours', 'B-Price', 'I-Cuisine', 'B-Dish', 'I-Amenity', 'I-Price', 'B-Restaurant_Name', 'I-Restaurant_Name', 'I-Dish']
        '''
        new_pred_label = []
        print(gold_label[0])
        print(pred_label)
        for pred in pred_label:
            ...
            #print(pred)
            for ele in pred:
                if ele not in allowed_label_list:
                    new_preds.append("O")
                else:
                    new_preds.append(ele)
            new_pred_label.append(new_preds)
        pred_label = new_pred_label
        '''
        #print(pred_label)
        pred_results += pred_label
        gold_results += gold_label
        save_preds += pred_label
        save_labels += gold_label
    if name == 'test':
        with open("./output/debug.txt", "w+") as f:
            for sent, labels, preds in zip(save_words, save_labels, save_preds):
                for w, l, p in zip(sent, labels, preds):
                    save_str = w+"\t"+l+"\t"+p+"\n"
                    f.write(save_str)
    ...
    label_list = data.label_alphabet.get_content()['instances']
    print("label list:%s" % (" ".join(label_list)))
    get_metrics(gold_results, pred_results,label_list)

    #get_metrics_BIOS(gold_results, pred_results)
...
def batchify_sequence_labeling_with_label(input_batch_list, gpu, if_train=True):
    ...
    return word_seq_tensor,feature_seq_tensors, word_seq_lengths, word_seq_recover, char_seq_tensor, char_seq_lengths, char_seq_recover, label_seq_tensor, mask, gazs_tensor, batch_t_matrix, batch_c_matrix, batch_l_matrix
...
def train(data):
    ...
    for idx in range(data.HP_iteration):
        ...
        batch_size = data.HP_batch_size
        batch_id = 0
        train_num = int(len(data.train_Ids))
        total_batch = train_num//batch_size+1
        for batch_id in range(total_batch):
            start = batch_id*batch_size
            end = (batch_id+1)*batch_size
            ...
            if not instance:
                continue
            batch_word, batch_features, batch_wordlen, batch_wordrecover, batch_char, batch_charlen, batch_charrecover, batch_label, mask, gazs, t_graph, c_graph, l_graph  = batchify_with_label(instance, data.HP_gpu, True, data.sentence_classification)
            instance_count += 1
            loss, tag_seq = model.calculate_loss(batch_word, batch_features, batch_wordlen, batch_char, batch_charlen, batch_charrecover, batch_label, mask, gazs, t_graph, c_graph, l_graph)
        ...
        speed, test_acc, test_p, test_r, test_f, _, _ = evaluate(data, model, "test")
        ...
        if data.seg:
            ...
            print("Test: time: %.2fs, speed: %.2fst/s; acc: %.4f, p: %.4f, r: %.4f, f: %.4f" % (
                test_cost, speed, test_acc, test_p, test_r, test_f))
        else:
            current_score = acc
            print("Dev: time: %.2fs speed: %.2fst/s; acc: %.4f"%(dev_cost, speed, acc))
            print("Test: time: %.2fs, speed: %.2fst/s; acc: %.4f" % (test_cost, speed, test_acc))
        ...
        if current_score > best_dev:
            ...
            best_p, best_r, best_f = test_p, test_r, test_f

        print("best test:  p: %.4f, r: %.4f, f: %.4f" % (best_p, best_r, best_f))
