    ...
    ### for sequence labeling data format i.e. CoNLL 2003
        for line in in_lines:
            if len(line) > 2:
                ...
                if len(pairs) >= 4:
                    ents = [int(ele) for ele in pairs[2].split("#")]
                    lens = [int(ele) for ele in pairs[3].split("#")]
                    #priors = [float(ele) for ele in pairs[4].split("#")]
                    gazs.append(ents)
                    #gaz_ids.append([ents, lens, priors])
                    gaz_ids.append([ents, lens])
                else:
                    gazs.append([])
                    gaz_ids.append([])
            else:
                if (len(words) > 0) and ((max_sent_length < 0) or (len(words) < max_sent_length)) :
                    instence_texts.append([words, features, chars, gazs, labels])
                    instence_Ids.append([word_Ids, feature_Ids, char_Ids,gaz_ids, label_Ids])
                words = []
                features = []
                chars = []
                labels = []
